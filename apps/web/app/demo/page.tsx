...
// ─── Config ────────────────────────────────────────────────────────────────────
const GROUPS: Record<UCGroup, { label: string; hex: string; icon: React.ElementType; ucs: number }> = {
  A: { label: 'A. Pre-Transaction Prevention',      hex: '#3B82F6', icon: Shield,   ucs: 9 },
  B: { label: 'B. Post-Transaction Investigation',  hex: '#8B5CF6', icon: Network,  ucs: 9 },
  C: { label: 'C. Analyst Dashboard',               hex: '#F59E0B', icon: Eye,      ucs: 7 },
  D: { label: 'D. Executive Dashboard',             hex: '#10B981', icon: BarChart3, ucs: 5 },
  E: { label: 'E. Learning Loop',                   hex: '#EC4899', icon: Brain,    ucs: 4 },
};
...
export default function DemoSimulatorPage() {
  ...
  return (
    <div style={{ backgroundColor: '#0A0F1E', minHeight: '100vh', padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        {(['A', 'B', 'C', 'D', 'E'] as UCGroup[])
          ...
          .map(g => {
            ...
            return (
              <div key={g}>
                <div
                  className="flex items-center justify-between rounded px-4 py-3 cursor-pointer mb-3 transition-colors"
                  style={{ backgroundColor: '#111827', border: '1px solid #1E2D45', borderLeft: `3px solid ${gc.hex}` }}
                  onMouseEnter={e => { const el = e.currentTarget as HTMLDivElement; el.style.backgroundColor = '#1A2235'; el.style.borderColor = '#2A3F60'; el.style.borderLeftColor = gc.hex; }}
                  onMouseLeave={e => { const el = e.currentTarget as HTMLDivElement; el.style.backgroundColor = '#111827'; el.style.borderColor = '#1E2D45'; el.style.borderLeftColor = gc.hex; }}
                  onClick={() => setCollapsedGroups(prev => { const n = new Set(prev); isOpen ? n.add(g) : n.delete(g); return n; })}>
                  <div className="flex items-center gap-3">
                    <gc.icon className="h-4 w-4" style={{ color: gc.hex }} />
                    <span className="text-sm font-semibold" style={{ color: gc.hex }}>{gc.label}</span>
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded" style={{ backgroundColor: '#1A2235', border: '1px solid #2A3F60', color: '#8899BB' }}>{doneCnt}/{gc.ucs} done</span>
                  </div>
                            ...
                            <div className="flex items-start gap-2">
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <span className="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded" style={{ color: gc.hex, backgroundColor: `${gc.hex}18` }}>{uc.id}</span>
                                  {state !== 'idle' && (
                    ...
                    return (
                      <tr key={uc.id} className="hover:bg-accent/30">
                        <td className="py-1.5 pr-3">
                          <span className="font-mono font-bold text-[10px] px-1.5 py-0.5 rounded" style={{ color: gc.hex, backgroundColor: `${gc.hex}18` }}>{uc.id}</span>
                        </td>
